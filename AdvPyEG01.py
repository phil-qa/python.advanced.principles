...
def delimit_types(data_to_delimit, delimit_point):
    '''
    separate the phonetic types into numeric and alphabetic
    :param data_to_delimit:source phonetic alphabet
    :param delimit_point: string value of list split
    :return:
    '''
    index_of_alphabet = data_to_delimit.index(delimit_point)
    numerics = data_to_delimit[0:index_of_alphabet]
    alphabet = data_to_delimit[index_of_alphabet:len(data_to_delimit)]
    return {"numerics": numerics, "alphabet": alphabet}


def develop_key_lists():
    '''
    develop two data structures for numbers and letters
    :return: dictionary of both
    '''
...
def develop_dictionary(keys, values):
    '''
    develop dictionary from keys and values
    :param keys:
    :param values:
    :return: dictionary of zipped keys and values
    '''
    return dict(zip(keys, values))

def main ():
    code_values = delimit_types(source, 'alpha')
