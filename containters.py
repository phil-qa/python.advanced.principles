...
'''
1.	Find the program containers.py.  In it is an implementation of collection classes Stack, Queue, and Deque, with a few tests in __main__.
Right now, iterating through any of these structures gives a TypeError. Your task is to implement __iter__ methods as generators, where appropriate.
Note that implementing __iter__ also implements the in operator.  This could also have been implemented using __contains__.
Hints:
Use the base-class node as the position indicator.
The methods all return None when they get to the end or when there is an empty container.
Stack:
	Start iteration at self._GetHead()
 	Yield the Data attribute of the node
	Get the next node by calling
self._GetNextFromLeft(current_node)
Queue:
	Start iteration at self._GetTail()
 	Yield the Data attribute of the node
	Get the next node by calling
		self._GetNextFromRight(current_node)
'''
